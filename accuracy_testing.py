...
def main():
    ...
    x_train, x_test, y_train, y_test = train_test_split(features_sentiment, target_sentiment, test_size=0.4)
    ...
    x_train, x_test, y_train, y_test = train_test_split(features_fakereview, target_fakereview, test_size=0.1)
    x_train, y_train = ADASYN(random_state=0).fit_resample(x_train, y_train)
    clf.fit(x_train, y_train)
    y_pred = clf.predict(x_test)
    print("FAKEREVIEW:")
